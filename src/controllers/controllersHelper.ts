...
const STATUS_CODE = Object.freeze({
  ...
  function okResponse(res:Response, text = STATUS_TEXT.OK) {
    return res.status(STATUS_CODE.OK).send(text);
  }
  
  function serverErrorResponse(res:Response, error, text = STATUS_TEXT.SERVER_ERROR) {
    console.error(error);
    return res.status(STATUS_CODE.SERVER_ERROR).send(text);
  }

  function unauthorizedRequestResponse(res:Response, text = STATUS_TEXT.UNAUTHORIZED) {
